...
export async function submitAppointmentForm(formData: AppointmentFormData): Promise<AppointmentResult> {
  try {
    ...
    return {
      success: true,
      message: `Thank you, ${formData.firstName}! Your appointment has been scheduled for ${formData.appointmentDate} at ${formData.appointmentTime}. We will contact you via your preferred method (${formData.preferredContact}) to confirm the details.`,
    }
  } catch (error) {
...
import { revalidatePath as revalidatePathStylist } from "next/cache"
...
export async function submitStylistForm(formData: StylistFormData): Promise<StylistResult> {
  try {
    ...
    // Revalidate the path to ensure fresh data
    revalidatePathStylist("/pages/ai-stylist")
